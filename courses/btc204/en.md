...
**> As a reminder:** The satoshi, often abbreviated to sat, is Bitcoin's smallest unit, comparable to the cent in fiat currencies.
...
### Change and merging in a Bitcoin transaction

The action of a Bitcoin transaction on UTXO can thus be compared to recasting a gold coin. Indeed, a UTXO is not divisible, but only fusible. This means that a user cannot simply divide a UTXO representing a certain amount in bitcoins into several smaller UTXOs. Instead, they must consume it entirely in a transaction to create one or more new UTXOs of arbitrary values in outputs, with the total value being less than or equal to the initial amount.
...
Bitcoin works in a similar way. Let's imagine that Alice has a UTXO of `10,000 SATS` and wishes to buy a baguette costing `4,000 SATS`. Alice will create a transaction with one UTXO of `10,000 SATS` as the input, which she will consume in full, and two UTXOs of `4,000 SATS` and `6,000 SATS` as outputs. The UTXO of `4,000 SATS` will be sent to the baker as payment for the baguette, while the UTXO of `6,000 SATS` will return to Alice as change. This returning UTXO, which returns to the original issuer of the transaction, is known as "change" in Bitcoin jargon.

![BTC204](assets/fr/013.webp)

Now, let's suppose that Alice doesn't have a single UTXO of `10,000 SATS`, but instead has two UTXOs of `3,000 SATS` each. In this situation, neither of the UTXOs individually is sufficient to send the baker `4,000 SATS`. Alice must therefore use both UTXOs of `3,000 SATS` as inputs in her transaction. This way, the total amount of the inputs reaches `6,000 SATS`, enabling her to cover the `4,000 SATS` payment to the baker. This method, in which multiple UTXOs are combined as inputs in a transaction, is often referred to as "merging".
...
**> Did you know?** Bitcoins generated by a coinbase transaction are subject to a maturity period of 100 blocks, during which they cannot be spent by the miner. This rule is designed to avoid complications linked to the use of newly created bitcoins on a chain that could later be rendered obsolete.
